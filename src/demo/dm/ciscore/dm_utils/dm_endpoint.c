...
extern unsigned char global_imei[30];
extern unsigned char global_imsi[30];
...
void dmSdkInit(void *DMconfig)
{
	...
	cis_memcpy(g_opt->szCMEI_IMEI, global_imei, sizeof(global_imei));

    cis_memset(g_opt->szIMSI, 0x00, sizeof(g_opt->szIMSI));
    //cissys_getIMSI(g_opt->szIMSI, sizeof(g_opt->szIMSI)); 
	cis_memcpy(g_opt->szIMSI, global_imsi, sizeof(global_imsi));
}
