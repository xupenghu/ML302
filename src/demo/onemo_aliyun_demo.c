...
void example_message_arrive(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    switch (msg->event_type) {
        case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
            ...
            onemo_printf("[ALIYUN]: Message Arrived:");
            onemo_printf("Topic  : %.*s", topic_info->topic_len, topic_info->ptopic);
            onemo_printf("Payload: %.*s", topic_info->payload_len, topic_info->payload);
            onemo_printf("\n");
            break;
        default:
            break;
...
int example_subscribe(void *handle)
{
    ...
    if (topic == NULL) {
        onemo_printf("[ALIYUN]: memory not enough\n");
        return -1;
    }
    ...
    if (res < 0) {
        onemo_printf("[ALIYUN]: subscribe failed\n");
        HAL_Free(topic);
        return -1;
    }
...
int example_publish(void *handle)
{
    ...
    if (res < 0) {
        onemo_printf("[ALIYUN]: publish failed, res = %d\n", res);
        HAL_Free(topic);
        return -1;
    }
...
{
    onemo_printf("[ALIYUN]: msg->event_type : %d\n", msg->event_type);
}

void onemo_test_aliyun(unsigned char **cmd,int len)
{
    onemo_printf("[ALIYUN]: aliyun demo start\n");
    ...
    if (NULL == pclient) {
        onemo_printf("[ALIYUN]: MQTT construct failed\n");
        return -1;
    }
    ...
    onemo_printf("[ALIYUN]: aliyun demo end\n");
}
#endif